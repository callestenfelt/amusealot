...
- NEVER run git push without explicit user request (applies to both feature branches and master). Exception: docs-only changes (see Branching)
...
- **Docs-only exception (added 2026-08-19):** commits that touch only documentation files (`nextstep.md`, `README.md`, `CLAUDE.md`, `docs/`) may be ff-merged to master and pushed to origin without asking. If a branch contains any code, template, or script change, the normal approval rules apply to the whole branch.
...
- **Dark mode / "white-on-white" rule:** every text-bearing element — `<td>`, `<span>`, **and `<a>`** — must carry an explicit `background-color` matching its section, on the *same element* as its `color:`. (Scope grew twice: `<td>`s in `fddc485`, spans in `979bdce`, and the last 14 anchors on 2026-08-28. As of then the template has **zero** unpaired elements; keep it that way.) This is the only thing that stops classic Word-engine Outlook for Windows desktop (and Windows Mail) from producing white-on-white — those clients run their own colour inversion and ignore `<style>` rules, so pairing background+text on one element makes the invert stay coherent. Section palette (redesign 2026-08-27): body + footer = `#F7F5F1` (warm paper, ink text — the footer is no longer a dark block), card panels (What's new box, News+Feedback band) = `#FAF9F6` with `#ddd8ce` hairline borders, ink = `#080229`, muted = `#5c5876`. Never set a cell's `color:` without its sibling `background-color:`, and keep both in the same section's palette.
